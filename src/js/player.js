...
(function (root, factory) {
  ...
  lunar.removeClass = function (elem, name) {
    const remove = elem.attr('class').replace(new RegExp(`(\\s|^)${name}(\\s|$)`, 'g'), '$2');
...
(function ($) {
  var _ = {
    ...
    cursorPoint(evt, el) {
      ...
      let playObject = el.find('svg').attr('id');
    ...
    angle(ex, ey) {
      ...
      theta += 90; // in our case we are animating from the top, so we offset by the rotation value;
      if (theta < 0) theta = 360 + theta; // range [0, 360)
      return theta;
    },

    setGraphValue(obj, val, el) {
      const audioObj = el.find(_.settings.audioObj);
      const { pc } = _.settings;
      const dash = pc - parseFloat(((val / audioObj[0].duration) * pc), 10);
    ...
    reportPosition(el, audioId) {
      const progress = el.find(_.settings.progress);
    ...
    init(options) {
      ...
      let template = ['<svg viewBox="0 0 100 100" id="playable" version="1.1" xmlns="http://www.w3.org/2000/svg" width="34" height="34" data-play="playable" class="not-started playable">',
        '<g class="shape">',
      ...
      let svgId = $(this).find('svg').attr('id');
      ...
      $(_.settings.controlsObj).on('click', (e) => {
        const el = $(e.currentTarget).closest($(_.settings.thisSelector));
        ...
        switch (obj.class.replace('playable', '').trim()) {
          case 'not-started':
            ...
            audioId.addEventListener('timeupdate', () => {
      ...
      $(_.defaults.audioObj).on('progress', function (e) {
        if (this.buffered.length > 0) {
          ...
          const cache = $(e.currentTarget).parent().find(_.settings.precache);
          const el = $(this).closest($(_.settings.thisSelector));
